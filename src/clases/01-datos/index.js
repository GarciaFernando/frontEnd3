...
console.log('Ej 2:')
let [primero, , , cuarto] = usuarios
console.log(primero.email, primero.mascotas)
console.log(cuarto.email, cuarto.mascotas)

console.log('Ej 3:')
let copia = [...primero.mascotas]
let mascotaNueva = { nombre: 'Nami', edad: 2, color: 'negro' }
copia.push(mascotaNueva)
console.log(copia)
console.log(primero.mascotas)

console.log('Ej 4:')
function getNameAndPets({ apodo, mascotas }) {
    return `El usuario ${apodo} tiene ${mascotas.length} mascotas`
}
console.log(getNameAndPets(primero))

console.log('Ej 5:')
console.log(
    usuarios.filter((x) => x.mascotas.length > 1).map((x) => getNameAndPets(x))
)
const totalMascotas = usuarios.reduce((acc, usuario) => {
    return acc + usuario.mascotas.length
}, 0)
console.log(`Hay un total de ${totalMascotas} mascotas.`)
